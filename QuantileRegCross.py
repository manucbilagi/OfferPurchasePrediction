...
for r in range(1):
	...
	for split_index, (train, test) in enumerate(cv):
		...
		dump_svmlight_file(train_data.iloc[train,:], ntrain_label, training_file, zero_based=False)
		dump_svmlight_file(train_data.iloc[test,:], np.zeros((train_data.iloc[test,:].shape[0],)), test_file, zero_based=False)
		print "hi"
		of = open( os.path.join(data_path, "vw_trainset.csv") ,"w")
		...
		training_file = os.path.join(data_path, "vw_trainset.csv")
		...
		pprint.pprint(pred_label_test7[:10])
		pred_label_test7=[1 if x>=0.5 else 0 for x in pred_label_test7]
		pprint.pprint(pred_label_test7[:10])
		for i in range(len(pred_label_test7)):
			if pred_label_test7[i]==y_test.values[i]:
				count=count+1
		print count
