...
for r in range(1):
	...
	for split_index, (train, test) in enumerate(cv):
		...
		ntrain_label = np.power(train_label_rep[train].values, 2./3. )
		...
		pprint.pprint(pred_label_test6[:10])
		#pred_label_test4= 1./(1.+np.exp(-pred_label_test4))
		pred_label_test6[pred_label_test6>=0.5]=1
		pred_label_test6[pred_label_test6<0.5]=0
		pprint.pprint(pred_label_test6[:10])
		for i in range(len(pred_label_test6)):
			if pred_label_test6[i]==y_test.values[i]:
				count=count+1
		print count
