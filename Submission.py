...
for r in range(3):
	...
	pprint.pprint(len(train2))
	print "the datatype of train1 and train2"
	print train1.__class__
	...
	train1_label = train1_data['label'] #pandas.core.series.Series
	...
	X1,y1=train1_data,train1_label
	...
	dtrain1 = xgb.DMatrix( train1_data.values, label=train1_label.values, weight=sample_weights) #xgboost.DMatrix
	dtrain2 = xgb.DMatrix( train2_data.values, label=train2_label.values)
	dtest = xgb.DMatrix( test_data.values)
	...
	############### BLEND
	Xx,yy= pred_train, train2_label
	pprint.pprint(Xx.shape)
	Xx_new=SelectKBest(f_classif, k=5).fit_transform(Xx,yy)
	pprint.pprint(Xx_new.shape)
	print Xx_new.__class__

	X1x,y1y=pred_test,test_label
	pprint.pprint(X1.shape)
	X1x_new=SelectKBest(f_classif,k=5).fit_transform(X1x,y1y)
	pprint.pprint(X1x_new.shape)
	print X1x_new.__class__
	
	print "training extra trees forest"
	clf1 = ExtraTreesClassifier(n_estimators=100, n_jobs=8)
	clf1.fit(Xx_new, train2_label)
	pred_label_test = clf1.predict_proba(X1x_new)[:,1] #numpy.ndarray
	#pred_label_test3 = clf.predict_proba(test_data)[:,1]
...
bagged_prediction = (predictions[0]+predictions[1]+predictions[2])/3.  #numpy.ndarray
